...
def process_neck_flexion(
    ...
    """
    Estimates neck flexion (side-on) as the angle between:
      - Torso vector: hip-midpoint → shoulder-midpoint
      - Head vector:  ear (preferred) → shoulder-midpoint (fallback to nose)
    Conventions:
      - Neutral standing ≈ small angle
      - Flexion increases as chin approaches chest
    """
    # Open video
    ...
    with mp_pose.Pose(static_image_mode=False, min_detection_confidence=0.5) as pose:
        while True:
            ...
            shoulder_mid = np.array([(shL.x + shR.x)/2.0, (shL.y + shR.y)/2.0, (shL.z + shR.z)/2.0], dtype=float)
            hip_mid      = np.array([(hpL.x + hpR.x)/2.0, (hpL.y + hpR.y)/2.0, (hpL.z + hpR.z)/2.0], dtype=float)

            # Head landmark: prefer ear on the recorded side; fallback to nose if low visibility
            if side.lower() == "right":
            ...
            # Vectors
            v_torso = shoulder_mid - hip_mid            # hip-mid → shoulder-mid (approx vertical)
            v_head  = head_xyz - shoulder_mid           # shoulder-mid → head (ear/nose)

            # Angle between vectors (clamped)
            ...
            if n_t == 0.0 or n_h == 0.0:
                ...
                angle_deg = float(np.degrees(np.arccos(cosv)))  # 0..180

            # Track extremes
            min_fx = min(min_fx, angle_deg)
            max_fx = max(max_fx, angle_deg)

            # Draw and label
            mp_drawing.draw_landmarks(frame, results.pose_landmarks, mp_connections)
            cv2.putText(
                frame,
                f"Neck Flex: {int(angle_deg)}\xb0",
                (10, 40),
    ...
    summary_data = {
        ...
        "side":        side,
        "min_angle":   round(min_fx, 2),
        "max_angle":   round(max_fx, 2),
        "rom":         round(rom, 2),
